...
class HanoiTowers:
    ...
    def solve(self):
        self.moves = []
        self.custom_hanoi(len(self.diameters), 7, 0, 6)
        self.animate_solution()

    def custom_hanoi(self, n, source, target, auxiliary):
        if n > 0:
            if source == 7:
                self.custom_hanoi(n-1, source, auxiliary, target)
                self.moves.append((source, auxiliary))
                self.custom_hanoi(n-1, auxiliary, target, source)
            elif source == 0:
                self.custom_hanoi(n-1, source, auxiliary, target)
                self.moves.append((source, auxiliary))
                self.custom_hanoi(n-1, auxiliary, target, source)
                ...
                if target == 0:
                    self.moves.append((source, auxiliary))
                    self.custom_hanoi(n-1, source, auxiliary, target)
                    self.moves.append((auxiliary, target))
                    self.custom_hanoi(n-1, target, source, auxiliary)
                else:
                    self.custom_hanoi(n-1, source, auxiliary, target)
                    self.moves.append((source, auxiliary))
                    self.custom_hanoi(n-1, auxiliary, target, source)
